...
public class QueryStaffTeamServiceImpl implements QueryStaffTeamService {
    ...
    public List<NameModel> searchByStaffOrTeamName(Page<NameModel> page, String teamName , String staffName) {
        ...
        if (teamName == null && staffName != null) {
            ...
            QueryWrapper<Staff> qw = new QueryWrapper<Staff>().like("firstName", staffName).or().like("lastName ", staffName);
            List<Staff> staffs = staffMapper.selectList(qw);
            if (staffs==null){
                return null;
            }
            for (Staff staff : staffs) {
                ...
                List<StaffTeam> staffTeams = staffTeamMapper.selectList(new QueryWrapper<StaffTeam>().eq("staffId", staff.getId()));
                // 多个小组的时候，返回拼接的小组名字
            ...
            team = teamMapper.selectOne(new QueryWrapper<Team>().eq("team_name",teamName));
        ...
        // 两者同时不为空
            ...
            Team team = teamMapper.selectOne(new QueryWrapper<Team>().eq("team_name",teamName));
            // 先找出唯一的Team
            // 通过对 firstName 以及 lastname 模糊查询，查找出所有有关的员工的信息
            List<Staff> staffs = staffMapper.selectList(new QueryWrapper<Staff>().like("firstName", staffName).or().like("lastName ", staffName));
            ...
            for (Staff staff : staffs) {
                ...
                StaffTeam staffTeam = staffTeamMapper.selectOne(new QueryWrapper<StaffTeam>().eq("staff_id",staff.getId()).eq("team_id",team.getId()));
                if (staffTeam == null){
            ...
            List<Staff> staffs = staffMapper.selectList(new QueryWrapper<Staff>());
            if (staffs==null||staffs.size()==0){
                return null;
            }
