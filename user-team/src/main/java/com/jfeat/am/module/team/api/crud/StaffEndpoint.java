...
import com.jfeat.am.module.team.api.model.TeamPermission;
...
public class StaffEndpoint {
    ...
    public Tip updateStaff(@PathVariable long id, @Valid @RequestBody StaffModel entity) {
        ...
        if (staff.getUserId().compareTo(userId) == 0){
            entity.setId(id);
            entity.setUserId(staff.getUserId());
            int affect = staffService.updateMaster(entity);
            return SuccessTip.create(affect);
        }else{
            return ErrorTip.create(PowerBusinessCode.PowerBusinessNoPermission.getCode(),
                    PowerBusinessCode.PowerBusinessNoPermission.getMessage());
        }
    }
    ...
    public Tip findCreatePeopleTeam(){
